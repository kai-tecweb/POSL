...
📌 固定URL・固定文言（v1.1 新規追加機能・完全版）
1. 固定URL設定（3件登録／1件選択／未使用可）

ユーザーは最大 3つの固定URL を登録できる。
登録したURLの中から 1つだけを「使用中URL」 として選択する。
さらに 「どれも使わない」 を選択できる。

登録数：最大3件

選択方式：ラジオボタン（4択：URL1 / URL2 / URL3 / 使わない）

activeIndex = -1 の場合は使用しない

使用中URLは通常投稿（20:00）の文末に付与

イベント投稿（08:00台）には付与しない
...
2. 固定文言設定（10文字以内×3件登録／1件選択／未使用可）

ユーザーは 10文字以内の固定文言を最大3件 登録できる。
登録した文言から 1件だけ「使用中文言」 を選択する。
さらに 「どれも使わない」 を選択可能。
...
選択方式：ラジオボタン（文言1 / 文言2 / 文言3 / 使わない）

activeIndex = -1 の場合は使用しない

使用中文言は通常投稿（20:00）の文末に付与
...
投稿全体が 280文字以内 に収まるように、以下の優先順位で削除する。

優先度（削除順）

固定URLを削除

それでも280文字を超える場合 → 固定文言も削除

それでも超える場合 → 本文を280文字以内にトリム（末尾カット）

対象外

activeIndex = -1 の場合は、そもそも文字数計算に含めない

5. 付与ポリシーまとめ

URL・文言の両方 未使用（activeIndex = -1） → 付与なし
